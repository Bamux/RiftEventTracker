...
def outputloop(unstable_events, language, zonenames, lfm):
    ...
    while True:
        if serverlocation == "log":
            ...
            if line:
                ...
                old_events = lofile_output(serverlocation, data_output, eventlist, zonenames, language, old_events)
...
def lofile_output(serverlocation, data_output, eventlist, zonenames, running_log, old_events):
    events = []
    if not running_log:
...
Thread(target=outputloop, args=(config_var['Settings']['unstable_events'], config_var['Settings']['language'],
                                zone_names, config_var['Settings']['lfm'])).start()
mainloop()
